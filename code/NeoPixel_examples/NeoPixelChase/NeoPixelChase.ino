/*
  ...
  This sketch turns on one neoPixel at a time in a strip.
...
const int pinNumber = 2;    // Pin number of the DIN line to the neoPixels
...
void loop() {
  ...
  for (int pixel = 0; pixel < pixelCount; pixel++) { 
    ...
    if (pixel > 0) {                          // if pixel > 0, 
      strip.setPixelColor(pixel - 1, 0);      // then turn off pixel 0
    } else {                                  // if pixel == 0,
      strip.setPixelColor(pixelCount - 1, 0); // then turn off highest number pixel
    }
